...
async def WriteMess(line,fn):
    try:
        await asyncio.sleep(line[2])
        rpnstr = f'{line[2]:.2f}: {line[0]}: {line[1]}'
        rpn(rpnstr)
        await fn.write(f'{rpnstr}\n')
    except Exception as _err:
        rpn(f'[red1]Ошибка {_err}')

async def main():
    try:
        FileName = os.path.join(r'C:\Dev\Asynchronous\solution.txt')
        async with aiofiles.open(FileName, mode='w', encoding = 'utf_8') as fn:pass
        async with aiofiles.open(FileName, mode='a', encoding = 'utf_8') as fn:
            tasks = [asyncio.create_task(WriteMess(line,fn)) for line in input_data]
