...
async def main():
    ...
    try:
        ...
        #--------------------------------------------------------------------------------
            # Создание трех задач
            tasks = [taskgroup.create_task(test_task(i)) for i in range(1, 4)]
            # [taskgroup.create_task(coro(), name=f'Задача_0{i}') for i in range(1, 4)]
        ...
        #--------------------------------------------------------------------------------
            # Создание и прямое получение результата задачи
            result = await taskgroup.create_task(some_coro(num))
        #--------------------------------------------------------------------------------
        ...
        # печать результата выполнения каждой задачи.
        for task in tasks:
            print(task.result())
